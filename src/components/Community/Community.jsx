...
const socket = io('https://code-circle-server-three.vercel.app/', { withCredentials: true });
...
const Community = () => {
  ...
  useEffect(() => {
    ...
    fetch('https://code-circle-server-three.vercel.app/api/messages', {
      credentials: 'include',
    })
      .then(res => {
  ...
  return (
    <div className="h-full flex flex-col" style={{ 
      ...
      {/* Chat Messages */}
      <div className="flex-1 overflow-y-auto p-4" style={{ background: 'rgba(17,24,39,0.8)', backdropFilter: 'blur(10px)' }}>
        {messages.map((msg) => {
          ...
          return (
            <div key={msg._id} className="mb-4 p-3 rounded-lg bg-[rgba(45,55,72,0.5)] text-gray-200">
              <div className="flex items-center gap-2 mb-1">
                {msg.sender === user?.email && user?.photoURL ? (
                  <img
                    src={user.photoURL}
                    alt={displayName}
                ...
                <div className="flex items-center justify-between w-full">
                  <span className="font-bold text-indigo-400">
                    {msg.sender === user?.email ? displayName : msg.sender}
                  </span>
              ...
              {msg.imageUrl && (
                <img src={`http://localhost:3000${msg.imageUrl}`} alt="Uploaded" className="mt-2 max-w-xs rounded" />
              )}
              <button
      ...
      <div className="p-4 bg-[rgba(30,41,59,0.3)] backdrop-blur-md border-t border-[rgba(255,255,255,0.1)]">
        ...
        {selectedImage && (
          <div className="mt-2 text-sm text-gray-400">
            ...
            <X size={16} className="ml-2 cursor-pointer inline" onClick={() => { setSelectedImage(null); fileInputRef.current.value = ''; }} />
          </div>
        )}
      </div>
    </div>
